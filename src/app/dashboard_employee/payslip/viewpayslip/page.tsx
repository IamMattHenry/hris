              ...
              {loading ? (
                ...
                <div className="payslip-content bg-white border border-[#E8D9C4] rounded-xl p-5 md:p-6 space-y-5 print:border-0 print:p-0 print:rounded-none">
                  <div className="text-center border-b border-gray-300 pb-4">
                    <h1 className="text-xl font-bold">{content.payslip.company_name || "Company Name"}</h1>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 text-xs">
                    <div className="space-y-1">
                      <p>
                        <span className="font-semibold">Employee:</span> {content.record.employee_name}
                      </p>
                      <p>
                      ...
                      <p>
                        <span className="font-semibold">Pay Schedule:</span> {content.run.pay_schedule?.toUpperCase()}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p>
                        <span className="font-semibold">Pay Period:</span> {content.run.pay_period_start} – {content.run.pay_period_end}
                      </p>
                      <p>
                        <span className="font-semibold">Run ID:</span> #{content.run.id}
                      </p>
                      <p>
                        <span className="font-semibold">Status:</span> {content.run.status?.toUpperCase()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 pt-3 border-t border-gray-300">
                    <div className="space-y-1.5 text-xs">
                      ...
                      <p>SSS (ER): {formatMoney(content.deductions.mandatoryContributions?.sss?.employerShare)}</p>
                      <p>PhilHealth (ER): {formatMoney(content.deductions.mandatoryContributions?.philHealth?.employerShare)}</p>
                      <p>Pag-IBIG (ER): {formatMoney(content.deductions.mandatoryContributions?.pagIbig?.employerShare)}</p>
                    </div>
                    <div className="flex flex-col items-end justify-end">
                      <p className="font-semibold text-base">NET PAY</p>
                      <p className="text-3xl font-bold text-[#3D1A0B]">{formatMoney(content.record.net_pay)}</p>
                    </div>
                  </div>
