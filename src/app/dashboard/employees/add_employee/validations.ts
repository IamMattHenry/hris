...
export const validateStep3 = (
  ...
  if (dependents.length === 0) {
    errors.dependents = "must have at least 1 dependent information is required";
  } else {
    dependents.forEach((dependent, index) => {
      // Validate dependent email if provided
      if (dependent.email && !/^[a-zA-Z0-9._%+-]+@gmail\.com$/i.test(dependent.email.trim())) {
        errors[`dependent_${index}_email`] = "Dependent email must be a valid Gmail address";
      }
      ...
      if (dependent.contactInfo && !/^(\+639|09)\d{9}$/.test(dependent.contactInfo.replace(/\s/g, ""))) {
        ...
        if (!/^[0-9\s\-\+\(\)]+$/.test(dependent.contactInfo)) {
          errors[`dependent_${index}_contact`] = "Dependent contact info format is invalid";
        }
      }
    });
  }
...
export const validateDependent = (
  ...
  if (contactInfo && !/^(\+639|09)\d{9}$/.test(contactInfo.replace(/\s/g, ""))) {
    errors.contactInfo = "Invalid format (must be 09XX XXX XXXX or +639XX XXX XXXX)";
  }
