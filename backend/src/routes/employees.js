...
// Update own employee profile (any authenticated user can update their own profile)
  router.put(
    ...
    [
      ...
      body('scheduled_days').optional().custom((value) => {
        let arr = value;
      ...
      body('scheduled_start_time').optional().matches(/^([01]\d|2[0-3]):([0-5]\d)(?::([0-5]\d))?$/).withMessage('scheduled_start_time is required (HH:MM or HH:MM:SS)'),
      body('scheduled_end_time').optional().matches(/^([01]\d|2[0-3]):([0-5]\d)(?::([0-5]\d))?$/).withMessage('scheduled_end_time is required (HH:MM or HH:MM:SS)')
    ],
    handleValidationErrors,
    updateEmployee
  );
